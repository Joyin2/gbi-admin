...
export default function EditHandicraftProduct({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function fetchHandicraftProduct() {
      try {
        const productDoc = await getDoc(doc(db, 'handicraftProducts', id));
        if (productDoc.exists()) {
          ...
          alert('Handicraft product not found');
          router.push('/admin/dashboard/handicraft');
        }
      } catch (error) {
        console.error('Error fetching handicraft product:', error);
        alert('Error loading handicraft product. Please try again.');
      } finally {
        setLoading(false);
      }
    }

    fetchHandicraftProduct();
  }, [id, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (image) {
        ...
        const filename = `handicraft-product-${isVideo ? 'videos' : 'images'}/${Date.now()}-${image.name.replace(/[^a-zA-Z0-9.]/g, '_')}`;
      ...
      // Update handicraft product in Firestore
      await updateDoc(doc(db, 'handicraftProducts', id), {
        name,
        description,
      ...
      setStatus({
        type: 'success',
        message: 'Handicraft product updated successfully!'
      });
      ...
      console.error('Error updating handicraft product:', error);
      setStatus({
        type: 'error',
        message: 'Failed to update handicraft product. Please try again.'
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">Edit Decorative Handicraft</h1>
        <button
          onClick={() => router.back()}
