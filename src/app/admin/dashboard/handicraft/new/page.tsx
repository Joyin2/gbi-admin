...
export default function NewHandicraftProduct() {
  ...
  const uploadImage = async (file: File): Promise<string> => {
    try {
      ...
      const isVideo = file.type.startsWith('video/');
      const storageRef = ref(storage, `handicraft-product-${isVideo ? 'videos' : 'images'}/${filename}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'handicraftProducts'), productData);
      
      setStatus({
        type: 'success',
        message: 'Handicraft product created successfully! Redirecting...'
      });
      ...
      if (error instanceof Error) {
        setStatus({
          type: 'error',
          message: error.message || 'Failed to create handicraft product. Please try again.'
        });
      } else {
        setStatus({
          type: 'error',
          message: 'Failed to create handicraft product. Please try again.'
        });
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">Add New Decorative Handicraft Product</h1>
        <button
          onClick={() => router.back()}
