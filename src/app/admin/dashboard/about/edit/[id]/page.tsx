...
export default function EditAboutSection() {
  ...
  // New state for multiple media files
  ...
  const fetchSectionData = async () => {
    try {
      setDataLoading(true);
      const docRef = doc(db, 'aboutSections', sectionId);
      ...
      if (docSnap.exists()) {
        ...
        setSectionType(data.sectionType || 'text');
        setContent(data.content || '');
        setCurrentMediaUrl(data.mediaUrl || '');
        setMediaType(data.mediaType || null);
        // Load existing multiple media files
        setCurrentMediaFiles(data.mediaFiles || []);
        ...
        setValueCards(data.valueCards || [{ icon: '', title: '', description: '' }]);
        setMissionCards(data.missionCards || [{ title: '', description: '' }]);
      } else {
        setStatus({
          type: 'error',
  ...
  const removeCurrentMediaFile = (index: number) => {
    setCurrentMediaFiles(prev => prev.filter((_, i) => i !== index));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (sectionType === 'video' && !media && !currentMediaUrl) {
      setStatus({
        type: 'error',
    ...
    if (sectionType === 'image' && !media && !currentMediaUrl && mediaFiles.length === 0 && currentMediaFiles.length === 0) {
      setStatus({
        type: 'error',
    ...
    try {
      ...
      let updatedMediaFiles = [...currentMediaFiles];
      ...
      if (mediaFiles.length > 0) {
        const newUploadedFiles = await uploadMultipleMedia(mediaFiles);
        updatedMediaFiles = [...updatedMediaFiles, ...newUploadedFiles];
      }
      ...
      const sectionData = {
        ...
        content: content,
        mediaUrl,
        mediaType: media ? mediaType : (currentMediaUrl ? mediaType : null),
        // Updated field for multiple media files
      ...
      await updateDoc(doc(db, 'aboutSections', sectionId), sectionData);
      ...
      setStatus({
        type: 'error',
        message: error instanceof Error ? error.message : 'Failed to update about section. Please try again.'
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Update the content and settings for this section.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="space-y-2">
                        <div className="grid grid-cols-6 gap-2 p-3 border border-gray-300 rounded-md bg-gray-50 max-h-40 overflow-y-auto">
                          {[
                            ...
                            { id: 'building', name: 'Building', svg: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4 16.5v-13h-.25a.75.75 0 010-1.5h12.5a.75.75 0 010 1.5H16v13h.25a.75.75 0 010 1.5h-3.5a.75.75 0 01-.75-.75v-2.5a.75.75 0 00-.75-.75h-2.5a.75.75 0 00-.75.75v2.5a.75.75 0 01-.75.75h-3.5a.75.75 0 010-1.5H4zm2-11a.5.5 0 01.5-.5h1a.5.5 0 01.5.5v1a.5.5 0 01-.5.5h-1a.5.5 0 01-.5-.5v-1zM6.5 9a.5.5 0 00-.5.5v1a.5.5 0 00.5.5h1a.5.5 0 00.5-.5v-1a.5.5 0 00-.5-.5h-1zm-.5 3.5a.5.5 0 01.5-.5h1a.5.5 0 01.5.5v1a.5.5 0 01-.5.5h-1a.5.5 0 01-.5-.5v-1zm.5 2.5a.5.5 0 00-.5.5v1a.5.5 0 00.5.5h1a.5.5 0 00.5-.5v-1a.5.5 0 00-.5-.5h-1z" clipRule="evenodd" /></svg> },
        ...
        <div>
          ...
          {/* Show existing multiple media files for image sections */}
          {sectionType === 'image' && currentMediaFiles.length > 0 && (
            <div className="mb-4">
              <h4 className="text-sm font-medium text-gray-700 mb-3">Current Media Files ({currentMediaFiles.length})</h4>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {currentMediaFiles.map((mediaFile, index) => (
                  <div key={index} className="relative group">
                    ...
                    <button
                      type="button"
                      onClick={() => removeCurrentMediaFile(index)}
                      className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      ×
                    </button>
                    <div className="mt-1 text-xs text-gray-600 text-center">
                      {mediaFile.type === 'video' ? '🎥' : '📷'} {mediaFile.filename}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {sectionType === 'image' && (
            <div className="space-y-4">
              ...
              {/* Preview New Multiple Media Files */}
              {mediaFilePreviews.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-3">New Media Files ({mediaFilePreviews.length})</h4>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-1 text-center">
                  ...
                  <div className="flex text-sm text-gray-600">
                    <label
                      ...
                      className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500"
                    >
                      <span>{currentMediaUrl ? 'Replace file' : 'Upload a file'}</span>
                      <input
                        id="media-upload"
                        type="file"
                        className="sr-only"
